...
import { useQueryClient, useMutation, type UseMutationOptions, type UseMutationResult } from "@tanstack/react-query";
...
export const enhancedFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    try {
        ...
        if (init?.body && typeof init.body === "object" && !(init.body instanceof FormData)) {
            ...
            const hasFileLikeFields = Object.keys(bodyObj).some((key) => key.toLowerCase().includes("image") ||
 ...
 * @internal
 */
async function uploadWithFormDataInternal<
...
export interface UploadWithFormDataConfig<
    P extends PathKey | string,
    M extends "post" | "put",
    TVariables extends { body: Record<string, unknown> } | { params: { path: { [key: string]: string } }; body: Record<string, unknown> },
    TContext = unknown
> {
    ...
    options?: Omit<UseMutationOptions<P extends PathKey ? ExtractSuccessResponse<P, M> : unknown, ApiError, TVariables, TContext>, "mutationFn">;
}
...
export function uploadWithFormData<
    P extends PathKey | string,
    M extends "post" | "put",
    TVariables extends { body: Record<string, unknown> } | { params: { path: { [key: string]: string } }; body: Record<string, unknown> } = M extends "post"
        ? { body: Record<string, unknown> }
...
>(
    ...
    return () => {
        ...
        return useMutation<P extends PathKey ? ExtractSuccessResponse<P, M> : unknown, ApiError, TVariables, TContext>({
            mutationFn: async (variables: TVariables): Promise<P extends PathKey ? ExtractSuccessResponse<P, M> : unknown> => {
