...
export default function PaymentScheduleClient({ reservationId, reservation }: PaymentScheduleClientProps) {
  ...
  if (isLoading) {
    return (
      <div>
        <HeaderPage title="Cronograma de Pagos" description="Cargando cronograma de pagos..." />
  ...
  if (error) {
    return (
      <div>
        <HeaderPage title="Cronograma de Pagos" description="Error al cargar el cronograma de pagos" />
        <ErrorGeneral />
      </div>
    );
  }
      ...
      <div className="mb-6">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbItem>
              <BreadcrumbLink asChild>
                <Link href={"/reservations"}>{reservation.clientName}</Link>
              </BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage>Cronograma de Pagos</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
      </div>
          ...
          <div>
            ...
            <div className="mt-1">
              {(() => {
                ...
                if (!statusInfo) {
                  return <span className="text-sm text-gray-900 dark:text-gray-100">{reservation.status}</span>;
                }
