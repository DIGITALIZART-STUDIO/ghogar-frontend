    ...
    <div className="space-y-6 font-montserrat">
      {showSuccess ? (
        <div className="flex flex-col items-center justify-center py-8">
          ...
          <h3 className="text-2xl font-bold mb-3 text-center text-foreground">¡Estado Actualizado!</h3>
          <p className="text-muted-foreground text-center text-lg">
            <span className="font-semibold text-foreground">{leadName}</span> ha sido actualizado correctamente
          </p>
          {selectedStatusOption && (
            <div className="mt-4 flex items-center gap-2">
              <Badge className={selectedStatusOption.badgeColor}>
                {selectedStatusOption && <selectedStatusOption.icon className="w-4 h-4 mr-1" />}
        ...
        <>
          ...
          <div className="text-center pb-4 border-b border-border">
            <div className="flex items-center justify-center gap-3 mb-3">
              <Badge className={currentStatusDetails.color}>
                <span className="flex items-center">
                  ...
                  <span className="ml-1 text-foreground">{currentStatusDetails.label}</span>
                </span>
              </Badge>
              {selectedStatusOption && (
                <>
            ...
            <h3 className="text-xl font-semibold mb-1 text-foreground">{leadName}</h3>
            <p className="text-sm text-muted-foreground">Selecciona el nuevo estado para este lead</p>
          </div>

          {/* Selección de estado */}
          <div className="space-y-3">
            <h4 className="font-semibold text-base text-foreground">Nuevo Estado</h4>
              ...
              {statusOptions.map((option) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      "p-4 border-2 rounded-lg cursor-pointer transition-all dark:ring-offset-background",
                      option.color,
                      isSelected ? "ring-2 ring-primary ring-offset-2" : ""
                    )}
                  >
                    <div className="flex items-start gap-3">
                      <div className={cn("p-2 rounded-lg bg-white dark:bg-background", option.iconColor)}>
                        <Icon className="w-5 h-5" />
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <h5 className="font-semibold text-foreground">{option.label}</h5>
            ...
            <div className="bg-muted/30 dark:bg-muted/10 rounded-lg p-4 border border-border">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-foreground">{currentStatusDetails.label}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Nuevo estado:</span>
                  <span className="font-medium text-foreground">{selectedStatusOption?.label}</span>
                </div>
                {requiresReason && selectedReason && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Razón:</span>
                    <span className="font-medium text-foreground">{selectedReasonOption?.label}</span>
                  </div>
                )}
              </div>
            </div>
