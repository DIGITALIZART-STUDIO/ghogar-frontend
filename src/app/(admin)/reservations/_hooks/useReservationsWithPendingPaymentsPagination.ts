...
import {
  ...
  PaymentMethod,
  ReservationStatus,
} from "@/app/(admin)/reservations/_types/reservation";
...
export function useReservationsWithPendingPaymentsPagination(page: number = 1, pageSize: number = 10) {
  ...
  const { search, filters, orderBy, orderDirection, setSearch, setFilter, handleOrderChange, resetFilters } =
    ...
    ); // Debounce manejado en data-table-toolbar
  ...
  const contractValidationStatusEnums =
    filters.contractValidationStatus?.length > 0
  ...
  const handleContractValidationStatusChange = useCallback(
    (values: Array<string>) => {
