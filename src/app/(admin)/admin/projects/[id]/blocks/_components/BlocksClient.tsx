...
import { Filter, Home, Search, Loader2, X } from "lucide-react";
    ...
    if (isLoading) {
        return (
            <div className="space-y-6">
                <HeaderPage title={`Manzanas - ${project?.name ?? "Sin nombre"}`} description={project?.location ?? "Sin ubicación"} />
                ...
                <div className="w-full gap-4">
                    ...
                    {search && (
                        <div className="mt-2 text-sm text-muted-foreground">
                            {blocks.length > 0 ? (
                                <span>
                                    {blocks.length} manzana{blocks.length !== 1 ? "s" : ""} encontrada{blocks.length !== 1 ? "s" : ""} para &quot;{search}&quot;
                                </span>
                            ) : (
