...
export default function EditReservationPageClient({ reservationId }: EditReservationPageClientProps) {
  ...
  } = useReservationById(reservationId);
  const {
    data: quotationData,
    error: quotationsError,
    isLoading: loadingQuotation,
  } = useQuotationByReservationId(reservationId);
  ...
  if (reservationError || !reservationData) {
    return (
      <div>
        ...
        <HeaderPage title="Editar Separación" description="Modifique la información de la separación de lote" />
        <ErrorGeneral />
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="-mx-4 flex-1 overflow-auto px-4 py-1 lg:flex-row lg:space-x-12 lg:space-y-0">
        <EditReservationPage reservationData={reservationData} quotationData={quotationData as Quotation} />
      </div>
    </div>
  );
}
