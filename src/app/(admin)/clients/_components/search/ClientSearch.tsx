...
export function ClientSearch({
  ...
  const { allClients, query, handleScrollEnd, handleSearchChange, search } = usePaginatedClientsWithSearch(10, {
    preselectedId,
    useCurrentUser,
    projectId,
  });

  const clientOptions: Array<Option<Client>> = useMemo(
    () =>
        ...
        component: (
          <div className="grid grid-cols-2 gap-2 w-full" title={client.name ?? "Sin nombre"}>
            <div className="flex flex-col gap-1">
              <span className="text-sm font-semibold truncate">{client.name ?? "Sin nombre"}</span>
            ...
            <div className="flex flex-col gap-1 items-end">
              ...
              >
                {client.type === "Natural" ? <User className="w-3 h-3 mr-1" /> : <Building2 className="w-3 h-3 mr-1" />}
