...
import { Home, IdCard, Mail, Pencil, Trash2, User, Users } from "lucide-react";
...
export function CoOwnersSection({ clientId, form }: CoOwnersSectionProps) {
  ...
  const updateSeparationCoOwner = () => {
    if (
      ...
      !newSeparationCoOwner.name ||
      !newSeparationCoOwner.dni ||
      !newSeparationCoOwner.address
    ) {
      toast.error("Por favor complete todos los campos obligatorios");
      return;
    }
      ...
      <div className="bg-card rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div className="p-4 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-medium text-slate-800 dark:text-slate-200">Copropietarios del Cliente</h2>
          </div>
        </div>
        ...
        <div className="p-4 space-y-6">
          ...
          {clientCoOwners.length > 0 && (
            <div className="space-y-3">
              <h3 className="text-sm font-medium text-slate-700 dark:text-slate-300">Copropietarios Registrados</h3>
              ...
              {selectedCoOwners.length > 0 && (
                <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                  <p className="text-sm text-blue-800 dark:text-blue-200">
                    <strong>{selectedCoOwners.length}</strong> copropietario(s) del cliente seleccionado(s) para la
                    separación
                  </p>
                </div>
              )}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 block">DNI *</label>
                <InputWithIcon
                  Icon={IdCard}
              ...
              <div>
                <label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 block">Teléfono</label>
                <PhoneInput
                  defaultCountry={"PE"}
              ...
              <div>
                <label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 block">Dirección *</label>
                <InputWithIcon
                  Icon={Home}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              <h4 className="text-sm font-medium text-slate-700 dark:text-slate-300">
                {editingSeparationCoOwnerIndex !== null
                  ? "Editar Copropietario de Separación"
                  : "Agregar Nuevo Copropietario de Separación"}
              </h4>
              ...
              <div className="flex items-center gap-2">
                ...
                {editingSeparationCoOwnerIndex !== null && (
                  <Button type="button" variant="outline" onClick={cancelSeparationEdit}>
                    Cancelar
                  </Button>
                )}
              </div>
          ...
          {(selectedCoOwners.length > 0 || separationCoOwners.length > 0) && (
            <div className="mt-4 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
              <p className="text-sm text-green-800 dark:text-green-200">
                <strong>{selectedCoOwners.length + separationCoOwners.length}</strong> copropietario(s) participarán en
                esta separación
                {selectedCoOwners.length > 0 && (
                  <span className="block text-xs mt-1">• {selectedCoOwners.length} del cliente</span>
                )}
                {separationCoOwners.length > 0 && (
                  <span className="block text-xs">• {separationCoOwners.length} específicos de la separación</span>
                )}
              </p>
            </div>
          )}
