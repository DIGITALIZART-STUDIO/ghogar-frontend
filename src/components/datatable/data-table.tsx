...
export function DataTable<TData, TValue>({
    ...
    }, [expansionMode, renderLateralContent, renderExpandedRow, selectedRow, onLateralToggle, onClickRow, isMobile, table, expanded]);
                                ...
                                <React.Fragment key={row.id}>
                                    <TableRow
                                        ...
                                        className={`transition-colors duration-150 ease-in-out hover:bg-muted/30 cursor-pointer ${
                                            expansionMode === "lateral" && selectedRow === row.original
                                                ? "bg-muted/50"
                                                : ""
                                        } ${
                                            expansionMode === "vertical" && row.getIsExpanded()
                                                ? "bg-muted/30"
                                                : ""
                                        }`}
                ...
                <div className="space-y-4">
                    ...
                    {lateralExpanded && selectedRow && (
                        <div className="border rounded-md bg-background animate-in fade-in-0 slide-in-from-bottom-2 duration-200 ease-out">
                            ...
                            <div className="p-4">
                                {renderLateralContent(selectedRow)}
                            </div>
                        </div>
                    )}
                </div>
        ...
        return (
            <div className="h-full">
                <ResizablePanelGroup direction="horizontal" className="h-full">
                    <ResizablePanel
                        defaultSize={lateralExpanded ? 100 - lateralPanelSize : 100}
                        minSize={30}
                    >
                        <TableComponent />
                    </ResizablePanel>
                    ...
                    <DrawerContent onClick={(ev) => ev.stopPropagation()}>
                        ...
                        <div className="flex-1 overflow-hidden">
                            <ScrollArea className={finalDrawerScrollClassName}>
                                <div className="px-4">
                                    {drawerRow && renderExpandedRow(drawerRow)}
                                </div>
                            </ScrollArea>
                        </div>
                    </DrawerContent>
