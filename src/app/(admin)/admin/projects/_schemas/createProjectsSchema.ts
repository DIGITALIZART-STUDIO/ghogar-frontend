...
export const projectSchema = z.object({
  ...
  defaultDownPayment: z.number().min(0).max(100).optional(),
  defaultFinancingMonths: z.number().min(1).optional(),
  ...
  projectImage: z
    ...
    .refine(
      (file) =>
        ["image/jpeg", "image/jpg", "image/png", "image/webp", "image/gif", "image/svg+xml"].includes(file.type),
      "Solo se permiten archivos de imagen (JPG, PNG, WEBP, GIF, SVG)"
    )
    .optional(),
  projectUrlImage: z.string().url().optional().nullable(),
});
