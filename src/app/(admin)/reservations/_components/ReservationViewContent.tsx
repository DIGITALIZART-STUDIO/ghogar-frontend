...
import { ReservationStatusLabels, PaymentMethodLabels, CurrencyLabels } from "../_utils/reservations.utils";
...
export default function ReservationViewContent({
    data,
    isLoading,
    daysLeft,
}: ReservationViewContentProps) {
    if (isLoading) {
        return (
        ...
        <div className="space-y-6 font-montserrat">
            ...
            <div className="rounded-lg bg-card p-6 text-card-foreground">
                <div className="flex items-center justify-between mb-4">
                    ...
                    {data.status && ReservationStatusLabels[data.status as ReservationStatus] ? (
                        <Badge variant="outline" className={ReservationStatusLabels[data.status as ReservationStatus].className}>
                            {React.createElement(ReservationStatusLabels[data.status as ReservationStatus].icon, {
                                className: "size-4 flex-shrink-0 mr-1",
                            })}
                            {ReservationStatusLabels[data.status as ReservationStatus].label}
                        </Badge>
                    ) : (
                ...
                <div className="mt-4 pt-4 border-t border-border flex items-center justify-between">
                    <div className="flex items-center">
                        ...
                        <span className="text-sm">
                            {data.reservationDate ? format(parseISO(data.reservationDate), "dd 'de' MMMM yyyy", { locale: es }) : "No disponible"}
                        </span>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p className={`font-medium ${isExpired ? "text-red-600" : ""}`}>
                            {data.expiresAt ? format(parseISO(data.expiresAt), "dd 'de' MMMM yyyy", { locale: es }) : "No especificado"}
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="flex items-center">
                        ...
                        <div>
                            ...
                            <p className="font-medium">
                                {data.reservationDate ? format(parseISO(data.reservationDate), "dd MMM yyyy", { locale: es }) : "No disponible"}
                            </p>
                        </div>
                    </div>
