...
import { LotStatus } from "@/app/(admin)/admin/projects/[id]/blocks/[blockId]/lots/_types/lot";
import {
  getLotStatusConfig,
  LotStatusConfig,
} from "@/app/(admin)/admin/projects/[id]/blocks/[blockId]/lots/_utils/lots.filter.utils";
                    ...
                    <RechartsPieChart>
                      ...
                      >
                        {(data.lotsByStatus ?? []).map((item, index) => {
                          const config = getLotStatusConfig(
                            (item.status as LotStatus) ?? ("defaultStatus" as LotStatus)
                          );
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div className="w-4 h-4 rounded-full" style={{ backgroundColor: fillColor }} />
          ...
          {!hasPerformanceData ? (
            ...
            <ResponsiveContainer height={450}>
              <ComposedChart data={data.monthlyPerformance} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                />
                <Bar dataKey="leads" fill="#facc15" name="Leads" radius={[4, 4, 0, 0]} opacity={0.95} />
                <Bar dataKey="quotations" fill="#fb923c" name="Cotizaciones" radius={[4, 4, 0, 0]} opacity={0.95} />
                <Line
                  type="monotone"
                  dataKey="sales"
                  stroke="#10b981"
