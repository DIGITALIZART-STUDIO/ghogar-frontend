import { Activity, AlertTriangle, CheckCircle, Clock, MapPin, Phone } from "lucide-react";
        ...
        <Card className="border-l-4 border-l-red-600">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              <Badge className="bg-red-600 hover:bg-red-700 text-white">{data.leadsKpi?.expiredLeads ?? 0}</Badge>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-green-600">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              <Badge className="bg-green-600 hover:bg-green-700 text-white">{data.leadsKpi?.completedLeads ?? 0}</Badge>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          {!data.recentLeads || data.recentLeads.length === 0 ? (
            <EmptyState icon={Activity} title="Sin leads recientes" description="No hay leads recientes para mostrar" />
          ) : (
            <div className="space-y-3">
              {(data.recentLeads ?? []).map((lead) => (
                <Card key={lead.id} className="border-l-4 border-l-slate-600 hover:shadow-md transition-shadow">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="flex-1">
                          <h4 className="font-medium text-slate-800 dark:text-slate-100">{lead.clientName}</h4>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          ...
                          <p className="text-xs text-slate-600 dark:text-slate-400">{lead.captureSource}</p>
                        </div>
                        <div className="text-right">
                        ...
                        <Badge className={`${getPriorityColor(lead.priority ?? "")} text-white`}>{lead.priority}</Badge>
                      </div>
                    </div>
                  </CardContent>
                </Card>
