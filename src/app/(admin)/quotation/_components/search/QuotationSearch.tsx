...
import { formatCurrency } from "../../_utils/quotations.utils";
...
export function QuotationSearch({
  ...
  const { allQuotations, query, handleScrollEnd, handleSearchChange, search } =
    usePaginatedAcceptedQuotationsWithSearch(10, preselectedId);

  const quotationOptions: Array<Option<SummaryQuotation>> = useMemo(
    () =>
      allQuotations.map((quotation) => ({
        ...
        component: (
          <div
            ...
            title={`${quotation.code} - ${quotation.clientName}`}
          >
            <div className="flex flex-col gap-1 flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-sm font-semibold truncate">{quotation.code}</span>
                <span className="text-sm text-muted-foreground truncate">- {quotation.clientName}</span>
              </div>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {quotation.finalPrice && (
                <Badge variant="outline" className="text-xs">
                  ...
                  {formatCurrency(quotation.finalPrice, quotation.currency)}
                </Badge>
              )}
