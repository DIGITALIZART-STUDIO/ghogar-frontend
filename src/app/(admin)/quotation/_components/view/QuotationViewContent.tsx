...
import { formatCurrency, QuotationStatusLabels } from "../../_utils/quotations.utils";
...
export default function QuotationViewContent({
  ...
  const downPaymentAmount = Math.round(((data?.finalPrice ?? 0) * (data?.downPayment ?? 0)) / 100);
      ...
      <div className="rounded-lg bg-card p-6 text-card-foreground">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="font-medium">{data.leadClientName}</p>
            </div>
          </div>
        ...
        <div className="mt-4 pt-4 border-t border-border flex items-center justify-between">
          ...
          <div className="flex items-center">
            ...
            <span className="text-sm">{daysLeft > 0 ? `${daysLeft} días restantes` : "Expirada"}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card rounded-lg p-5 border-l-4 border-chart-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            ...
            <p className="font-medium">{data.blockName}</p>
          </div>
          ...
          <div>
            ...
            <p className="font-medium">{data.areaAtQuotation} m²</p>
          </div>

          <div>
            <p className="text-sm text-muted-foreground">Precio/m²</p>
            <p className="font-medium">{formatCurrency(data.pricePerM2AtQuotation ?? 0, currency)}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-card rounded-lg p-5 border-l-4 border-primary">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="font-medium">{formatCurrency(data?.totalPrice ?? 0, currency)}</span>
            </div>

            <div className="flex items-center justify-between">
              <span className="text-sm text-muted-foreground">Descuento</span>
              <span className="font-medium text-chart-1">-{formatCurrency(data.discount ?? 0, currency)}</span>
            </div>

            <div className="flex items-center justify-between pt-2 border-t border-border">
              <span className="font-medium">Precio Final</span>
              <span className="font-bold text-lg">{formatCurrency(data?.finalPrice ?? 0, currency)}</span>
            </div>
          </div>
          ...
          <div className="mt-4 pt-3 border-t border-border">
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              ...
              <span>${data.exchangeRate} USD</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card rounded-lg p-5 border-l-4 border-chart-3">
          ...
          <div className="space-y-4">
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-sm text-muted-foreground">
                  Inicial ({data.downPayment}
                  %)
                </span>
                <span className="font-medium">{formatCurrency(downPaymentAmount, currency)}</span>
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-sm text-muted-foreground">A Financiar</span>
                <span className="font-medium">{formatCurrency(data.amountFinanced ?? 0, currency)}</span>
              </div>
              <Progress
            ...
            <div className="pt-3 border-t border-border">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="font-medium">{data.monthsFinanced} meses</div>
                </div>

                <div className="text-right">
                  <span className="text-sm text-muted-foreground">Cuota Mensual</span>
                  <div className="font-bold text-lg">{formatCurrency(calculateMonthlyPayment(), currency)}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="font-medium">{data.advisorName}</p>
            </div>
          </div>
