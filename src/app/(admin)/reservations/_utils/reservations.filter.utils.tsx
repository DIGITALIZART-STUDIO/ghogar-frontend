...
import { PaymentMethodLabels, ReservationStatusLabels } from "./reservations.utils";

// Generar componentes de icono a partir de ReservationStatusLabels
const ReservationStatusIcons = Object.fromEntries(
  Object.entries(ReservationStatusLabels).map(([reservationStatus, config]) => {
...
const PaymentMethodIcons = Object.fromEntries(
  Object.entries(PaymentMethodLabels).map(([paymentMethod, config]) => {
    const IconComponent: React.FC<{ className?: string }> = ({ className }) => {
      const Icon = config.icon;
      return <Icon className={cn(className, config.className)} />;
    };
