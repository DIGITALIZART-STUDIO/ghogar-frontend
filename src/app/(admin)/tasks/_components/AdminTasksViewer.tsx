...
export function AdminTasksViewer({
  ...
  const filteredTasks = data.filter((task) => {
    ...
    if (searchQuery) {
      ...
      const matchesType = getTaskLabel(task.type).toLowerCase().includes(query);
  ...
  const overdueTasks = filteredTasks.filter(
    (task) => !task.isCompleted && isBefore(parseISO(task.scheduledDate), currentDate)
  );

  // Ordenar tareas por fecha (más recientes primero)
  const sortedTasks = [...filteredTasks].sort(
    (a, b) => parseISO(b.scheduledDate).getTime() - parseISO(a.scheduledDate).getTime()
  );
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center p-8 text-center">
                <p className="text-muted-foreground">No se encontraron tareas con los filtros seleccionados.</p>
                              ...
                              >
                                ...
                                <div className="flex-1">
                                  <div className="flex items-center justify-between mb-1">
                                    <Badge className={getTaskClasses(task.type)}>{getTaskLabel(task.type)}</Badge>
                                    {isOverdue && <Badge variant="destructive">Retrasada</Badge>}
                                    {task.isCompleted && (
                                      <Badge
                                        variant="outline"
                                  ...
                                  <div className="flex flex-wrap justify-between items-center mt-2 text-sm text-muted-foreground dark:text-white">
                                    <div>
                                      Lead: <span>{getLeadName(task.lead?.clientId ?? "")}</span>
                                    </div>
                                    <div className="flex items-center gap-1">
                                      Asesor: <span>{getAdvisorName(task.assignedToId)}</span>
                                    </div>
                                  </div>
                                </div>
                              </div>
